...
class Program
{
    static void Main(string[] args)
    {
        ...
        var userPosts = context.BlogComments
        ...
        {
            Console.WriteLine($"{userPost.Username}: {userPost.CommentCount}");
        }
            ...
            .Select(post => new
        {
            ...
            .OrderByDescending(x => x.LastComment.CreatedDate);
        foreach (var post in posts)
        {
            Console.WriteLine($"{post.Title}: {post.LastComment.CreatedDate}, {post.LastComment.Text}");
        }
        //ToDo: write a query and dump the data to console
        // Expected result (format could be different, e.g. object serialized to JSON is ok):
        // Post2: '2020-03-06', '4'
            ...
            .Select(post => new
        {
            ...
            .GroupBy(latestComment => latestComment.LatestComment.UserName)
            .Select(group => new
        {
            Name = group.Key,
            LatestCommentCount = group.Count()
        })
            .ToList();

        foreach (var commentInfo in lastCommentsPerUser)
        {
            Console.WriteLine($"{commentInfo.Name}: {commentInfo.LatestCommentCount}");
        }
